...
function getTenderReason(tender: Tender, highRiskCompanyIds: Set<string>): string {
  if (tender.winnerCompanyId && highRiskCompanyIds.has(tender.winnerCompanyId)) {
    return "Winner is linked to a high-risk company profile.";
  }

  const baseline = tender.averageMarketPrice || tender.budget;
  if (baseline > 0 && tender.finalPrice > baseline) {
    const delta = Math.round(((tender.finalPrice - baseline) / baseline) * 100);
    return `Final price is ${delta}% above the available baseline.`;
  }

  if (!tender.winnerCompanyId) return "No winner has been published yet.";
  return "No public high-risk signal detected.";
}
        ...
        <div className="container flex min-h-[88px] items-center justify-between gap-6 py-5">
          <Link to="/public" className="flex items-center gap-3">
            ...
            <span>
              ...
              <span className="block text-lg font-bold">Transparency Monitor</span>
            </span>
          </Link>
        ...
        <div className="container grid gap-8 py-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-risk-medium-border bg-risk-medium-bg px-3 py-1 text-xs font-semibold uppercase tracking-wider text-risk-medium">
              ...
              Citizen-facing corruption signals
            </div>
            <h1 className="max-w-3xl text-4xl font-bold tracking-tight text-foreground">
              Public view of tender awards, suspicious companies, and money at risk.
            </h1>
            <p className="mt-4 max-w-2xl text-base leading-7 text-muted-foreground">
              This page turns procurement data into an open evidence surface: citizens can inspect winners, price anomalies, and company risk patterns without entering the admin system.
            </p>
          </div>
          ...
          <div className="rounded-lg border border-border bg-background p-5 shadow-elevation-sm">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Risk exposure</p>
                <p className="mt-1 text-2xl font-bold text-foreground">{riskCoverage}% flagged for review</p>
              </div>
            ...
            <div className="mt-4 grid grid-cols-3 gap-3 text-center">
              <div className="rounded-md border border-risk-high-border bg-risk-high-bg p-3">
                ...
                <p className="text-xs text-risk-high">High</p>
              </div>
              <div className="rounded-md border border-risk-medium-border bg-risk-medium-bg p-3">
                ...
                <p className="text-xs text-risk-medium">Medium</p>
              </div>
              <div className="rounded-md border border-risk-low-border bg-risk-low-bg p-3">
                ...
                <p className="text-xs text-risk-low">Low</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="container space-y-6 py-8">
        {state === "partial" && (
          <div className="rounded-lg border border-risk-medium-border bg-risk-medium-bg px-4 py-3 text-sm text-risk-medium">
            Some public data could not be loaded. If the backend keeps risk endpoints admin-only, expose public read-only endpoints for this page.
          </div>
        )}
        ...
        {state === "error" ? (
          <div className="rounded-lg border border-risk-high-border bg-risk-high-bg p-8 text-center">
            ...
            <h2 className="text-lg font-semibold text-risk-high">Public data is not available</h2>
            <p className="mt-2 text-sm text-risk-high/80">
              The backend needs public read-only tender and risk endpoints for unauthenticated visitors.
            </p>
          </div>
        ) : (
          <>
            <div className="grid gap-4 md:grid-cols-3">
              <PublicMetric
                ...
                hint="Published procurement records visible in the transparency view."
                icon={Landmark}
              />
              <PublicMetric
                label="Money monitored"
                value={formatCompactCurrency(monitoredBudget)}
                hint="Combined budget or award value currently visible to citizens."
                icon={TrendingUp}
                tone="medium"
              />
              <PublicMetric
                ...
                hint="Companies with critical risk signals in procurement history."
                icon={ShieldAlert}
                tone="high"
              />
            </div>
              ...
              <div className="rounded-lg border border-border bg-card shadow-sm">
                <div className="border-b border-border p-5">
                  <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                    <div>
                      <h2 className="text-lg font-semibold text-foreground">Public Tender Register</h2>
                      <p className="mt-1 text-sm text-muted-foreground">
                        Search awards and inspect the public reason behind each risk label.
                      </p>
                    </div>
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <div className="relative">
                        ...
                        <Input
                          ...
                          placeholder="Search tenders, winners, agencies..."
                ...
                {state === "loading" ? (
                  <div className="p-12 text-center text-sm text-muted-foreground">Loading public register...</div>
                ) : filteredTenders.length === 0 ? (
                  <div className="p-12 text-center text-sm text-muted-foreground">No tenders match the current filters.</div>
                ) : (
                      ...
                      <article key={tender.id} className="p-5 transition-colors hover:bg-muted/30">
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 text-sm text-muted-foreground">
                              {tender.organization} · published {formatDate(tender.publishedAt)}
                            </p>
              ...
              <aside className="space-y-4">
                <div className="rounded-lg border border-border bg-card p-5 shadow-sm">
                  <div className="mb-4 flex items-center gap-2">
                    ...
                    <h2 className="text-base font-semibold text-foreground">Companies Under Review</h2>
                  </div>
                  <div className="space-y-3">
                    {state === "loading" ? (
                      <p className="text-sm text-muted-foreground">Loading companies...</p>
                    ) : featuredCompanies.length === 0 ? (
                      <p className="text-sm text-muted-foreground">No company risk data is public yet.</p>
                    ) : (
                        ...
                        <div key={company.id} className="rounded-lg border border-border bg-background p-4">
                          ...
                          <p className="text-sm leading-5 text-muted-foreground">
                            {company.suspicionFlags[0]?.message ??
                              `${company.totalWins} wins and ${company.failedProjects} failed projects recorded.`}
                          </p>
                        </div>
                      ))
                    )}
                ...
                <div className="rounded-lg border border-border bg-gradient-header p-5 text-primary-foreground shadow-elevation-sm">
                  <h2 className="text-base font-semibold">For procurement officials</h2>
                  <p className="mt-2 text-sm leading-6 text-primary-foreground/70">
                    Use the private dashboard to investigate evidence trails, block risky awards, and prepare audit reports.
                  </p>
